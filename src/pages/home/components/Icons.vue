<template>
	<div class="icons">
		<swiper :options="swiperOption">
			<swiper-slide v-for="(icon, index) in pages" :key="index">
...
<script>
	export default {
		...
		computed: {
			pages () {
				const pages = []
				this.iconList.forEach((item, index) => {  //把一位数组拆解为二维数组
					const page = Math.floor(index/8) //页码,如index为2 2/8向下取整为0,0是第一个页面
					if (!pages[page]) {
						pages[page] = []
						console.log(11)
					}
